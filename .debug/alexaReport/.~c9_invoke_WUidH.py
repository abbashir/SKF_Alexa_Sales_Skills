...
# testing
def testing_intent_response(intent):
    session_attributes = {}

       # get device id
    sys_object = intent.request_envelope.context.system
    device_id = sys_object.device.device_id
    device_id = 1
     
    # email = "https://api.amazonalexa.com/v2/accounts/~current/settings/Profile.name"
    card_title = "test"
    speech_output = "Your email is--- {}".format(str(device_id))
    reprompt_text = "Would you like to know any other report, like sales or stock"
    should_end_session = False
    return build_response(session_attributes, build_speechlet_response(
        card_title, speech_output, reprompt_text, should_end_session))
...
def on_intent(intent_request, session):
    ...
    # Outstanding - Total
        ...
        return testing_intent_response(intent)    
