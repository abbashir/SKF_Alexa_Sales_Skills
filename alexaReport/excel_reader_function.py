...
try:
    ...
    # server time
    fmt = '%Y-%m-%d %H:%M:%S'
    upload_time = datetime.strptime(upload_time, fmt)
    
    #present time
    UTC = pytz.utc
    timeZ_Kl = pytz.timezone('Asia/Dhaka')
    dt_Kl = datetime.now(timeZ_Kl)
    present_time = dt_Kl.strftime('%Y-%m-%d %H:%M:%S')
    present_time = datetime.strptime(present_time, fmt)
    
    #calculation
    time_delta = (present_time - upload_time)
    total_seconds = time_delta.total_seconds()
    diff_minutes = total_seconds / 60
    print(diff_minutes)
    
    
    if status == 1 and diff_minutes > 10:
        print("primary")
...
#create
# table.put_item(Item= {'id': 1, 'status': 0})

#update
# table.update_item(Item= {'id': 1, 'status': 1})
...
print("upload time: ",get_time_stamp())
